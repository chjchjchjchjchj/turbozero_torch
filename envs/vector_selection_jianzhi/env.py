...
class VectorSelectionJianzhiEnv(Env):
    def __init__(self,
        ...
        self.cached_legal_actions = torch.ones((self.parallel_envs, self.board_size), dtype=torch.bool, device=device, requires_grad=False)
    ...
    def is_terminal(self):
        return (self.cached_legal_actions.sum(dim=1, keepdim=True) == 0).flatten()
    ...
    def get_legal_actions(self) -> torch.Tensor:
        ...
        for env_idx in range(self.parallel_envs):
            ...
            if len(unselected_indices) == 0:
                legal_actions[env_idx] = False
                continue
            selected_vectors = self.all_vectors[selected_indices]
            unselected_vectors = self.all_vectors[unselected_indices]

            cos_matrix = unselected_vectors @ selected_vectors.T
            valid_rows = self.find_valid_rows(cos_matrix, self.valid_cos_arr)
            self.cached_legal_actions[env_idx, unselected_indices[~valid_rows]] = False
            if not valid_rows.any():
                legal_actions[env_idx] = False
                continue
            legal_actions[env_idx] = self.cached_legal_actions[env_idx]
            
            # 贪心策略剪枝
            valid_cos_indices = unselected_indices[valid_rows]
            valid_cos_unselected_vectors = unselected_vectors[valid_rows]

            count = torch.zeros(valid_cos_unselected_vectors.shape[0], device=self.device)
            batch_size = 1000
            ...
            for i in trange(num_batches):
                ...
                batch_count = count_valid_rows(batch_cos_matrix, self.valid_cos_arr)
                count[start:end] = batch_count
            k = min(1, count.shape[0])
            topk_values, topk_indices = torch.topk(count, k=k)
            legal_actions[env_idx] = False
            legal_actions[env_idx, valid_cos_indices[topk_indices]] = True
    ...
    def load_node(self, load_envs: torch.Tensor, saved: Tuple[torch.Tensor, torch.Tensor, torch.Tensor]):
        ...
        self.cached_legal_actions = saved_cached_legal_actions.clone() * load_envs_expnd_cached_legal_actions + self.cached_legal_actions * (~load_envs_expnd_boards)
